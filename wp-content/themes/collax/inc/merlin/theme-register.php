...
function collax_get_registered_purchase_code() {
	return get_option( 'tp_envato_purchase_code' );
}


function collax_register_theme() {
	add_submenu_page( 'themes.php', 'Register Theme', 'Register Theme', 'manage_options', 'register-theme', 'collax_register_theme_options' );
}
add_action( 'admin_menu', 'collax_register_theme' );

function collax_register_theme_options() {
	if ( !current_user_can( 'manage_options' ) )  {
		wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
	}

	if(empty(collax_get_registered_purchase_code())){
		...
		echo '<h2 class="hndle ui-sortable-handle">'.esc_html__('Register Theme','collax').'</h2>';
		echo '</div>';
		echo '<p style="max-width: 500px;">'.esc_html__('You\'re almost done. Just one more step. In order to gain full access to all demos, 
		premium plugins and support, please register your theme\'s purchase code.','collax').'</p>';
		echo '<h4>'.esc_html__('Your Envato Purchase Code','collax').'</h4>';
		...
		echo '<p>';
		echo '<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">'.esc_html__('Where to find the code?','collax').'</a>';
		echo '</p>';
		
		echo '<p>';
		echo '<label>';
		echo '<input type="checkbox">'.esc_html__('I confirm that, according to the Envato License Terms, I am licensed to use the purchase code for a single project. Using it on multiple installations is a copyright violation.','collax').'</label>';
		echo '<a href="https://themeforest.net/licenses/terms/regular" target="_blank">'.esc_html__('License details.','collax').'</a>';
		echo '</p>';
		
		echo '<p class="tp-actions">';
		echo '<button class="button button-primary">'.esc_html__('Register Theme','collax').'</button>';
		echo '</p>';
		echo '</div>';
		echo '</form>';
	} else {
					...
					<div class="welcome-panel-column-container">
						<div class="welcome-panel-column">
							...
							<div class="welcome-panel-column-content">
								<h3>Theme Support</h3>
								<p><?php echo esc_html__('Envato allows 1 license for 1 project located on 1 domain. It means that 1 purchase key can be used only for 1 site. Each additional site will require an additional purchase key.','collax'); ?></p>
		...
		if( is_wp_error($verify_purchase_code) ){
			...
			if( !empty($data) ){
				...
				if( $metas['purchase_code'] == $purchase_code && $metas['client_domain'] == $_SERVER['HTTP_HOST'] ){
					echo '<div class="notice notice-success"><p>'.esc_html__('The theme has already been active.','collax').'</p></div>';
				...
				if( $response_status == 400 || $response_status == 404 || $response_status == 401 ){
					...
					echo '<div class="notice notice-success"><p>'.esc_html__('The theme registered succesfully','collax').'</p></div>';
					update_option( 'tp_envato_purchase_code', $purchase_code );
					update_option( 'tp_envato_purchase_item_id', $verify_purchase_code['item']['id'] );
				}
...
function collax_is_theme_registered() {
	$purchase_code =  collax_get_registered_purchase_code();
...
function collax_tgmpa_filter_action_links( $action_links, $item_slug, $item, $view_context ) {
	$source = ! empty( $item['source'] ) ? $item['source'] : '';

	// Prevent installing theme's premium plugins.
	if ( 'External Source' === $source && ! collax_is_theme_registered() ) {
		$action_links = array(
			'collax_registration_required' => sprintf( __( '<a style="color: #ff0000;" href="%s">Register theme to unblock it</a>', 'collax' ), esc_url( admin_url( 'themes.php?page=register-theme' ) ) ),
		);
	}
...
$collax_tgmpa_prefix = ( defined( 'WP_NETWORK_ADMIN' ) && WP_NETWORK_ADMIN ) ? 'network_admin_' : '';
add_filter( 'tgmpa_' . $collax_tgmpa_prefix . 'plugin_action_links', 'collax_tgmpa_filter_action_links', 10, 4 );

/**
 * Admin Notice
 */

function collax_notice_for_activation() {
	if(empty(collax_get_registered_purchase_code())){
		echo '<div class="notice notice-warning">
			<p>' . sprintf(
			esc_html__( 'Enter your Envato Purchase Code to receive Theme and plugin updates %s', 'collax' ),
			'<a href="' . admin_url('themes.php?page=register-theme') . '">' . esc_html__( 'Enter Purchase Code', 'collax' ) . '</a>') . '</p>
		</div>';
	}
}
add_action('admin_notices', 'collax_notice_for_activation');