...
class Merlin {
	...
	protected function license() {
        if(collax_get_registered_purchase_code()) {
	...
	protected function plugins() {
        if(empty(collax_get_registered_purchase_code())) {
            echo "<p>".esc_html__('Please verify the purchase code first.','collax')."</p>";
			return new WP_Error( 'collax_theme_api_error', "Please verify the purchase code first. Automatic registration is not possible." );
        }
	...
	public function _ajax_activate_license() {
		...
		switch ($responseCode) {
			case 404: 
				wp_send_json(
					array(
						'success' => false,
						'message' => 'Your purchase code is not valid!',
					)
				);
				break;
			case 403: 
			...
			case 401: 
				wp_send_json(
					array(
						'success' => false,
						'message' => 'The personal token is invalid or has been deleted',
					)
				);
				break;
		}
		...
		if ($body === false && json_last_error() !== JSON_ERROR_NONE) {
			...
			$check_is_already_exists = wp_remote_get(
				"https://api.themepure.net/wp-json/wp/v2/purchase/?meta_key=purchase_code&meta_value={$license_key}&_fields=acf"
			);
			...
			if( !empty($data) ){
				$metas =  $data[0]['acf'];
				if( $metas['purchase_code'] == $license_key && $metas['client_domain'] == $_SERVER['HTTP_HOST'] ){
					update_option( 'tp_envato_purchase_code', $license_key );
					update_option( 'tp_envato_purchase_item_id', $body['item']['id'] );
					wp_send_json(
						array(
							'success' => true,
							'message' => 'The theme has already been active.',
						)
					);
				}else{
					...
					wp_send_json(
						array(
							'success' => false,
							'message' => 'This theme already active in different domain with the same license key!',
						)
					);
				}
			}else{
				...
				if( $response_status == 400 || $response_status == 404 || $response_status == 401 ){
					...
					wp_send_json(
						array(
							'success' => true,
							'message' => 'The theme registered succesfully',
						)
					);
