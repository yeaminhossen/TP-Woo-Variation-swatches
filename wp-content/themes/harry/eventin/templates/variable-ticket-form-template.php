<div class="etn-single-page-ticket-count-text-holder">
    <?php
        if ( !isset($event_options["etn_hide_seats_from_details"]) ) {
            ...
            <div class="etn-form-ticket-text">
                ...
                if( $etn_ticket_unlimited ){
                    echo esc_html__( "This event offers unlimited tickets", 'harry' );
                }else {
                    echo esc_html($etn_left_tickets) . esc_html__(' seats remaining', 'harry');
                }
                ?>
            </div>
            <?php
        ...
        if( !isset($event_options["etn_hide_attendee_count_from_details"]) ){
            ?>
            <div class="etn-form-ticket-text">
                <?php echo esc_html( $total_sold_ticket ) . esc_html__(" Attendees so far.", 'harry'); ?>
            </div>
            <?php
        }
    ?>
...
<form method="post" class="etn-event-form-parent" data-etn_uid="<?php echo esc_html($unique_id); ?>">
    ...
    <div class="etn-row etn-item-row">
        <div class="etn-qty-field etn-col-lg-6">
            <label for="etn_product_qty">
                <?php echo esc_html__('Quantity', 'harry'); ?>
            </label>
        ...
        <div class="etn-price-field etn-col-lg-6">
            <label for="etn_product_price">
                <?php echo isset($event_options["etn_price_label"]) && ( "" != $event_options["etn_price_label"]) ? esc_html($event_options["etn_price_label"]) : esc_html__('Price', 'harry'); ?>
            </label>
    ...
    <div class="etn-total-price">
        <?php echo esc_html__('Total price', 'harry'); ?>
        <?php 
    ...
    if ($show_form_button === false) {
        ?>
        <small><?php echo esc_html__('Event already expired!', 'harry'); ?></small>
        <?php
    } else {
        if (!isset($event_options["etn_purchase_login_required"]) || (isset($event_options["etn_purchase_login_required"]) && is_user_logged_in())) {
            ?>
            <input name="submit" class="etn-btn etn-primary etn-add-to-cart-block" type="submit" value="<?php $cart_button_text = apply_filters( 'etn_event_cart_button_text', esc_html__("Add to cart", 'harry') ); echo esc_html( $cart_button_text ); ?>" />
            <?php
        } else {
            ?>
            <small>
            <?php echo esc_html__('Please', 'harry'); ?> <a href="<?php echo wp_login_url( get_permalink( ) ); ?>"><?php echo esc_html__( "Login", 'harry' ); ?></a> <?php echo esc_html__(' to buy ticket!', 'harry'); ?>
            </small>
            <?php
        }
    }
